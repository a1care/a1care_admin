...
import { Gift, Users, CheckCircle2, Search, Loader2, ChevronLeft, ChevronRight, TrendingUp, Clock, AlertCircle, Settings, Save } from "lucide-react";
...
const partyName = (p?: PartyRef | string) => p && typeof p === "object" ? p.name || p.mobileNumber || "Unknown" : "Unknown";
const partyMobile = (p?: PartyRef | string) => p && typeof p === "object" ? p.mobileNumber || "" : "";
...
export function ReferralsPage() {
  ...
  const approveReferralMutation = useMutation({
    mutationFn: async (id: string) => {
      await api.put(`/referral/${id}/approve`);
    },
    onSuccess: () => {
  ...
  const handleSaveConfig = () => {
    ...
    updateConfigMutation.mutate({
      customerRewardAmount: cReward,
      partnerRewardAmount: pReward
    });
  };
  ...
  const filtered = items.filter(r => {
    ...
    return r.referralCode?.toLowerCase().includes(q) || partyName(r.referrerId).toLowerCase().includes(q) || partyName(r.refereeId).toLowerCase().includes(q) || partyMobile(r.referrerId).includes(q) || partyMobile(r.refereeId).includes(q);
  });
  ...
  const statCards = [{
    ...
    label: "Pending",
    value: Math.max(0, (data?.total ?? 0) - (data?.rewarded ?? 0)),
    icon: <Clock size={14} />,
    color: "text-amber-600 dark:text-amber-400"
  }, {
    label: "Total Paid",
    value: `₹${data?.totalRewardPaid ?? 0}`,
  ...
  return (
    <div className="space-y-6 animate-in">
        ...
        <PageBanner 
            ...
            subtitle="Configure referral rewards and approve incoming partner referrals."
        />
                    ...
                    <tbody className="divide-y divide-[var(--border-color)]">
                        <tr className="hover:bg-[var(--bg-main)] transition-colors">
                            <td className="py-3.5 px-4">
                                <div className="flex items-center rounded-lg shadow-sm border border-[var(--border-color)] overflow-hidden max-w-[200px] bg-[var(--card-bg)] focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all">
                                    <span className="flex items-center h-9 px-3 bg-[var(--bg-main)] text-[var(--text-muted)] font-semibold border-r border-[var(--border-color)] text-sm">₹</span>
                                    <input type="number" value={customerReward} onChange={e => setCustomerReward(e.target.value)} className="w-full h-9 bg-transparent px-3 text-sm font-bold text-[var(--text-main)] outline-none" placeholder="100" />
                                </div>
                            </td>
                            <td className="py-3.5 px-4">
                                <div className="flex items-center rounded-lg shadow-sm border border-[var(--border-color)] overflow-hidden max-w-[200px] bg-[var(--card-bg)] focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all">
                                    <span className="flex items-center h-9 px-3 bg-[var(--bg-main)] text-[var(--text-muted)] font-semibold border-r border-[var(--border-color)] text-sm">₹</span>
                                    <input type="number" value={partnerReward} onChange={e => setPartnerReward(e.target.value)} className="w-full h-9 bg-transparent px-3 text-sm font-bold text-[var(--text-main)] outline-none" placeholder="100" />
                                </div>
                            </td>
                            <td className="py-3.5 px-4 text-right">
                                <button onClick={handleSaveConfig} disabled={updateConfigMutation.isPending} className="h-9 px-4 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-colors shadow-sm ml-auto whitespace-nowrap disabled:opacity-70">
        ...
        <div className="grid grid-cols-4 gap-3">
            {statCards.map(s => <div key={s.label} className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-4 text-left">
  ...
  flexShrink: 0
    ...
    boxSizing: "border-box"
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-sm">
                ...
                <table className="w-full text-left min-w-[800px]">
                    ...
                    <tbody className="divide-y divide-[var(--border-color)]">
                        {isLoading ? <tr>
                                ...
                                <td colSpan={8} className="py-20 text-center">
                            ...
                            </tr> : filtered.map((r, index) => <tr key={r._id} className="hover:bg-[var(--bg-main)] transition-colors group">
                                    ...
                                    <td className="py-3.5 px-4 text-right">
                                        {r.status === "PENDING" && <button onClick={async () => {
              const isConfirmed = await confirm({
                title: "Credit Reward",
                message: "Are you sure you want to credit this reward to the referrer's wallet?",
                confirmText: "Credit",
                type: "info"
              });
              if (isConfirmed) {
            ...
            }} disabled={approveReferralMutation.isPending} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-semibold transition-colors disabled:opacity-50 inline-flex items-center gap-1">
            ...
            {totalPages > 0 && <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--border-color)] bg-[var(--card-bg)]">
