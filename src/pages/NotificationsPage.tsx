...
import { Send, Users, User, Bell, CheckCircle2, Search, X, Info, History, Calendar, UserSearch as UserSearchIcon, ShieldCheck, Trash2, Loader2, Plus, ChevronLeft, ChevronRight } from "lucide-react";
...
export function NotificationsPage() {
  ...
  return (
    <div className="space-y-6 animate-in">
      {/* ── Page Header ── */}
      <PageBanner title="Push Notifications" subtitle="Broadcast targeted alerts to users, partners, and the entire platform.">
        <div className="flex items-center gap-3 shrink-0">
          ...
          }} className="flex items-center gap-1.5 h-9 px-4 bg-white/15 hover:bg-white/25 border border-white/30 text-white text-xs font-semibold rounded-xl transition-all backdrop-blur-sm">
            <Plus size={15} />
            <span>Create</span>
          </button>
          <div className="flex gap-1 bg-white/10 border border-white/20 p-1 rounded-xl backdrop-blur-sm">
            <button onClick={() => setActiveTab('broadcast')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all ${activeTab === 'broadcast' ? "bg-white/20 text-white shadow-sm" : "text-white/70 hover:text-white"}`}>
              sended notification
            </button>
            <button onClick={() => setActiveTab('intelligence')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all ${activeTab === 'intelligence' ? "bg-white/20 text-white shadow-sm" : "text-white/70 hover:text-white"}`}>
              recived notifation
            </button>
          </div>
        </div>
      ...
      {/* ── Search User Modal ── */}
      {isSearchOpen && <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150 flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] bg-[var(--bg-main)]">
              ...
              <button onClick={() => setIsSearchOpen(false)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--card-bg)] border border-[var(--border-color)] transition-all">
                <X size={16} />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div className="relative">
                ...
                <input autoFocus type="text" placeholder="Search by name or mobile number..." className="w-full h-10 pl-10 pr-4 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg text-xs text-[var(--text-main)] placeholder:text-[var(--text-muted)] outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all font-medium" value={userSearchTerm} onChange={e => setUserSearchTerm(e.target.value)} />
              </div>
              <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                {searching ? <div className="flex items-center justify-center py-8 text-blue-500 gap-2">
                    <Loader2 size={16} className="animate-spin" />
                    <span className="text-xs font-semibold">Searching Database...</span>
                  </div> : searchResults && searchResults.length > 0 ? searchResults.map(user => <button key={user._id} onClick={() => selectUser(user)} className="w-full flex items-center justify-between p-3 rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)] hover:border-blue-500 hover:bg-[var(--card-bg)] transition-all text-left">
                      <div>
                    ...
                    </button>) : userSearchTerm.length >= 3 ? <div className="text-center py-8 text-[var(--text-muted)]">
                  ...
                  </div> : <div className="text-center py-8 text-[var(--text-muted)]">
            ...
            <div className="px-6 py-4 border-t border-[var(--border-color)] flex justify-end bg-[var(--bg-main)]">
              <button onClick={() => setIsSearchOpen(false)} className="h-9 px-4 border border-[var(--border-color)] text-[var(--text-main)] text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-[var(--border-color)] transition-all">
                Close
              </button>
            </div>
          </div>
      ...
      {isComposeOpen && <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150 flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] bg-[var(--bg-main)]">
              ...
              <button onClick={() => setIsComposeOpen(false)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--card-bg)] border border-[var(--border-color)] transition-all">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="space-y-1.5">
                ...
                label: "Individual",
                icon: Search
              }].map(item => <button key={item.id} type="button" onClick={() => handleAudienceChange(item.id as Audience)} className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all gap-1 cursor-pointer
                        ${audience === item.id ? "bg-blue-600 border-blue-600 text-white shadow-sm" : "bg-[var(--bg-main)] border-[var(--border-color)] text-[var(--text-muted)] hover:border-blue-400 hover:text-[var(--text-main)]"}`}>
                      <item.icon size={15} />
              ...
              {(audience === "patients" || audience === "partners" || audience === "admins") && <div className="space-y-1.5">
                ...
                setRecipientName("");
                    ...
                    <button type="button" onClick={() => setTargetScope("specific")} className={`flex-1 py-1 text-xs font-semibold uppercase tracking-wider rounded-md transition-all
              ...
              {/* Target Search Box */}
              {targetScope === "specific" && <div className="space-y-1.5 p-3 bg-[var(--bg-main)] rounded-lg border border-[var(--border-color)] animate-in fade-in">
                  ...
                  <button type="button" onClick={() => setIsSearchOpen(true)} className="w-full h-9 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-lg px-3 flex items-center justify-between text-xs hover:border-blue-500 transition-colors">
              ...
              <div className="space-y-1">
                ...
                <input className="w-full h-10 px-3 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg text-sm text-[var(--text-main)] placeholder:text-[var(--text-muted)] outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all font-semibold" placeholder="e.g. Special health checkup discount alert" value={title} onChange={e => setTitle(e.target.value)} required />
              </div>

              <div className="space-y-1">
                <label className="block text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wider">Message Body</label>
                <textarea className="w-full h-24 p-3 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg text-sm text-[var(--text-main)] placeholder:text-[var(--text-muted)] outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all resize-none font-semibold" placeholder="Enter message text..." value={body} onChange={e => setBody(e.target.value)} required />
              </div>
            </div>

            {/* Footer */}
            <div className="px-6 py-4 border-t border-[var(--border-color)] flex justify-end gap-2 bg-[var(--bg-main)]">
              <button type="button" onClick={() => setIsComposeOpen(false)} className="h-9 px-4 border border-[var(--border-color)] text-[var(--text-main)] text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-[var(--border-color)] transition-all">
                Cancel
              </button>
              <button disabled={sendMutation.isPending || !title || !body || targetScope === 'specific' && !recipientId} onClick={() => sendMutation.mutate()} className="h-9 px-5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-colors flex items-center justify-center gap-1.5 shadow-sm disabled:opacity-50">
                {sendMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <>
                    <Send size={13} />
      ...
      {activeTab === 'broadcast' ? (/* Notification Log List full screen */
    <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-[var(--border-color)] bg-[var(--bg-main)] flex items-center justify-between">
            ...
            <button onClick={() => refetchHistory()} className="text-xs text-blue-500 hover:underline font-semibold">
              Refresh Log
            </button>
          </div>
            ...
            <table className="w-full text-left min-w-[700px]">
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {isLoadingHistory ? <tr>
                    ...
                    </tr> : history.length > 0 ? history.map((item: any, index: number) => <tr key={item._id} className="hover:bg-[var(--bg-main)] transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold border
                          ${item.fcmStatus === 'sent' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : item.fcmStatus === 'failed' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>
                  ...
                  minute: '2-digit'
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <button onClick={() => deleteNotificationMutation.mutate(item._id)} disabled={deleteNotificationMutation.isPending} className="w-8 h-8 rounded-lg border border-rose-200 dark:border-rose-500/20 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 hover:bg-rose-600 hover:text-white dark:hover:bg-rose-500 dark:hover:text-white inline-flex items-center justify-center transition-colors" title="Delete history log">
                          <Trash2 size={13} />
                        </button>
                      </td>
                    </tr>) : <tr>
          ...
          {/* History Pagination */}
          {history.length > 0 && <div className="flex items-center justify-between px-4 py-3 bg-[var(--card-bg)] border-t border-[var(--border-color)] shadow-sm">
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={() => setHistoryPage(p => Math.max(1, p - 1))} disabled={historyPage === 1} className="w-8 h-8 flex items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-main)] hover:bg-[var(--bg-main)] disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                  <ChevronLeft size={14} />
                </button>
                <button onClick={() => setHistoryPage(p => p + 1)} disabled={history.length < 10} className="w-8 h-8 flex items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-main)] hover:bg-[var(--bg-main)] disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                  <ChevronRight size={14} />
                </button>
              </div>
            </div>}
        </div>) : (/* Intelligence log section */
    <div className="space-y-6">
          <div className="flex items-center justify-between bg-[var(--card-bg)] border border-[var(--border-color)] p-5 rounded-xl shadow-sm">
            ...
            <button onClick={() => clearAlertsMutation.mutate()} className="h-9 px-4 border border-[var(--border-color)] text-[var(--text-muted)] hover:text-red-500 hover:border-red-200 text-xs font-bold uppercase tracking-wider rounded-lg transition-all shrink-0">
              Clear All Alerts
            </button>
          </div>
          ...
          <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-sm text-left">
                ...
                <table className="w-full text-left min-w-[700px]">
                  ...
                  <tbody className="divide-y divide-[var(--border-color)]">
                    {isLoadingAlerts ? <tr>
                        ...
                        </tr> : alerts.length > 0 ? alerts.map((alert: any, index: number) => <tr key={alert._id} onClick={() => handleAlertClick(alert.refType)} className="hover:bg-[var(--bg-main)] transition-colors cursor-pointer group" title="Click to inspect this item">
                          <td className="py-3.5 px-4 text-xs font-semibold text-[var(--text-muted)]">
                          ...
                          <td className="py-3.5 px-4">
                            <div className="w-8 h-8 rounded-lg bg-[var(--bg-main)] border border-[var(--border-color)] flex items-center justify-center text-[var(--text-muted)]">
                              {alert.refType === 'ServiceRequest' ? <Calendar size={15} className="text-blue-500" /> : alert.refType === 'Partner' ? <ShieldCheck size={15} className="text-emerald-500" /> : <Bell size={15} className="text-amber-500" />}
                            </div>
                          </td>
                          <td className="py-3.5 px-4">
                            <div className="flex items-center gap-2">
            ...
            {/* Alerts Pagination */}
            {alerts.length > 0 && <div className="flex items-center justify-between px-4 py-3 bg-[var(--card-bg)] border-t border-[var(--border-color)] shadow-sm">
                <div className="flex items-center gap-4">
                  <p className="text-xs text-[var(--text-muted)]">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => setAlertsPage(p => Math.max(1, p - 1))} disabled={alertsPage === 1} className="w-8 h-8 flex items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-main)] hover:bg-[var(--bg-main)] disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                    <ChevronLeft size={14} />
                  </button>
                  <button onClick={() => setAlertsPage(p => p + 1)} disabled={alerts.length < 10} className="w-8 h-8 flex items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-main)] hover:bg-[var(--bg-main)] disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                    <ChevronRight size={14} />
                  </button>
                </div>
              </div>}
