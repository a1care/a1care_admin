...
import { Banknote, Clock, CheckCircle2, XCircle, Search, Filter, ChevronLeft, ChevronRight, Loader2, Building2, User, CreditCard, X, TrendingUp, Eye, Check } from "lucide-react";
interface Payout {
  _id: string;
  staffId: {
    _id: string;
...
export function PayoutsPage() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({
      ...
      return api.put(`/admin/payouts/${id}`, {
        status,
        adminNote: note,
        fromStatus
      });
  ...
  return (
    <div className="space-y-6 animate-in">
        <PageBanner title="Partner Settlements" subtitle="Manage and reconcile partner payouts and bank settlements.">
            <div className="flex flex-wrap gap-1 bg-white/10 backdrop-blur-md border border-white/20 p-1 rounded-xl w-fit">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] p-5 rounded-xl flex flex-col gap-2 relative overflow-hidden text-left">
                ...
                <div className="text-2xl font-bold text-[var(--text-main)]">₹{pendingTotal.toLocaleString('en-IN')}</div>
            ...
            {/* Stat 2: Average */}
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] p-5 rounded-xl flex flex-col gap-2 relative overflow-hidden text-left">
                <div className="flex items-center gap-1.5 text-xs font-medium text-blue-600 dark:blue-400">
                    <TrendingUp size={14} />
                    Average Settlement
                </div>
                <div className="text-2xl font-bold text-[var(--text-main)]">₹{averagePayout.toLocaleString('en-IN')}</div>
                <div className="text-[11px] text-[var(--text-muted)] mt-1">
      ...
      flexShrink: 0
        ...
        boxSizing: "border-box"
                    ...
                    <table className="w-full text-left min-w-[900px]">
                        ...
                        <tbody className="divide-y divide-[var(--border-color)]">
                            {isLoading ? <tr>
                                ...
                                </tr> : payouts.length > 0 ? payouts.map((payout, index) => {
              const isPending = payout.status === "PENDING";
              return <tr key={payout._id} className="hover:bg-[var(--bg-main)] transition-colors group">
                      ...
                      year: 'numeric'
                                            ...
                                            <td className="py-3.5 px-4">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold border
                                                    ${payout.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400' : payout.status === 'REJECTED' ? 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-500/10 dark:text-rose-400' : 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400'}`}>
                                            ...
                                            <td className="py-3.5 px-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    {/* VIEW details */}
                                                    <button onClick={() => setInspectPayout(payout)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-500/10 border border-[var(--border-color)] hover:border-blue-300 transition-all" title="View Details">
                                                        <Eye size={14} />
                                                    </button>
                                                    {/* APPROVE AND REJECT */}
                                                    {isPending && <>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--border-color)]">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={() => setPage(p => Math.min(Math.max(1, totalPages), p + 1))} disabled={page >= Math.max(1, totalPages)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-main)] hover:bg-[var(--bg-main)] disabled:opacity-40 disabled:cursor-not-allowed transition-all">
            ...
            {inspectPayout && <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] bg-[var(--bg-main)]">
                            ...
                            <button onClick={() => setInspectPayout(null)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--card-bg)] border border-[var(--border-color)] transition-all">
                                <X size={16} />
                            </button>
                        </div>
                            ...
                            <div className="flex items-center justify-between p-4 bg-[var(--bg-main)] rounded-xl border border-[var(--border-color)]">
                                ...
                                <div className="text-right">
                                    ...
                                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-[11px] font-semibold border
                                        ${inspectPayout.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : inspectPayout.status === 'REJECTED' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                                        {inspectPayout.status}
                                    </span>
                                </div>
                            </div>

                            {inspectPayout.adminNote && <div className="p-4 bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/20 rounded-xl">
            ...
            {/* ── Rejection Modal ── */}
            {selectedPayout && <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] bg-[var(--bg-main)]">
                            ...
                            <button onClick={() => setSelectedPayout(null)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--card-bg)] border border-[var(--border-color)] transition-all">
                                <X size={16} />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <textarea className="w-full h-24 p-3 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg text-sm text-[var(--text-main)] outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all resize-none font-semibold" placeholder="Explain why this request is being rejected (e.g. Invalid bank details)..." value={adminNote} onChange={e => setAdminNote(e.target.value)} required />
                            </div>
                        </div>

                        {/* Footer */}
                        <div className="px-6 py-4 border-t border-[var(--border-color)] flex justify-end gap-2 bg-[var(--bg-main)]">
                            <button onClick={() => setSelectedPayout(null)} className="h-9 px-4 border border-[var(--border-color)] text-[var(--text-main)] text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-[var(--border-color)] transition-all">
                                Cancel
                            </button>
                            <button onClick={() => {
                              if (!adminNote.trim()) {
                              ...
                              updateStatusMutation.mutate({
                                ...
                                status: 'REJECTED',
                                note: adminNote,
                                fromStatus: selectedPayout.status
                              });
                            }} disabled={updateStatusMutation.isPending || !adminNote.trim()} className="h-9 px-5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-rose-600">
