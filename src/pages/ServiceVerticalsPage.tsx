...
import { Stethoscope, Syringe, Ambulance, Truck, FlaskConical, LayoutGrid, ArrowRight, Settings2, ShieldCheck, Plus, X, Trash2, Edit2 } from "lucide-react";
...
export function ServiceVerticalsPage() {
  ...
  const handleSubmitVertical = async (e: React.FormEvent) => {
    ...
    // Refresh local state
    const newList = verticals.map(v => v.id === verticalData.id ? {
      ...v,
      ...verticalData
    } : v);
    if (!editingVertical) {
      setVerticals([...newList, {
        ...verticalData,
        icon: verticalData.imageUrl ? null : LayoutGrid
      }]);
    } else {
      setVerticals(newList);
    }
  ...
  return <div className="space-y-8 animate-in text-left items-start relative">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-[var(--card-bg)] p-8 rounded-3xl border border-[var(--border-color)] shadow-sm text-left items-start">
                <div className="space-y-2 text-left items-start">
                    <div className="w-full mb-6"><PageBanner title="Service Verticals" /></div>
        ...
        return <div key={vertical.id} className="group relative bg-[var(--card-bg)] p-8 rounded-[32px] border border-[var(--border-color)] hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 cursor-pointer overflow-hidden flex flex-col h-full text-left items-start">
              ...
              toast.success("Vertical removed.");
                            ...
                            <div onClick={() => navigate(`/service-categories?type=${vertical.id}`)} className="w-full flex-col h-full items-start">
                                ...
                                <div className={`w-16 h-16 rounded-2xl bg-${colorClass}-50 dark:bg-${colorClass}-500/10 flex items-center justify-center text-${colorClass}-600 mb-6 group-hover:rotate-6 transition-transform overflow-hidden shadow-inner`}>
                                    {vertical.imageUrl ? <img src={vertical.imageUrl} alt={vertical.title} className="w-full h-full object-cover" /> : <IconComponent size={32} />}
                                </div>
        ...
        setIsModalOpen(true);
            ...
            {/* Addition Modal */}
            {isModalOpen && <div className="modal-overlay fixed inset-0 z-[100] flex items-center justify-center p-6">
                        ...
                        <div className="p-10 space-y-8">
                            ...
                            <form className="space-y-6" onSubmit={handleSubmitVertical}>
                                <div className="space-y-2">
                                    ...
                                    <input className="w-full h-14 bg-slate-50 dark:bg-slate-800/50 border-none rounded-2xl px-6 text-slate-900 dark:text-white font-bold placeholder:text-slate-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all shadow-inner" placeholder="e.g. Wellness & Yoga" value={title} onChange={e => setTitle(e.target.value)} required />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Description</label>
                                    <textarea className="w-full h-24 bg-slate-50 dark:bg-slate-800/50 border-none rounded-2xl px-6 py-4 text-slate-900 dark:text-white font-bold placeholder:text-slate-400 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all shadow-inner resize-none" placeholder="Briefly describe the operational scope of this new vertical..." value={desc} onChange={e => setDesc(e.target.value)} required />
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-3 px-2">
                                        {['blue', 'indigo', 'emerald', 'rose', 'amber', 'purple'].map(c => <button key={c} type="button" onClick={() => setColor(c)} className={`w-8 h-8 rounded-full bg-${c}-500 shadow-lg transition-all ${color === c ? 'scale-125 ring-4 ring-white shadow-xl' : 'opacity-40 hover:opacity-100'}`} />)}
                                    </div>
                                </div>
