...
import { Plus, Trash2, Tag, X, Search, ChevronLeft, Star, LayoutGrid, CheckCircle2, Image, UploadCloud, Stethoscope, Syringe, FlaskConical, Ambulance, Edit2, Bell, History } from "lucide-react";
...
export function ServiceChildServicesPage() {
  ...
  const filtered = childServices?.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="space-y-6 animate-in">
        ...
        <div className="flex items-start lg:items-center gap-3 flex-col lg:flex-row">
            <button onClick={() => navigate(`/service-subcategories?categoryId=${activeCatId}`)} className="w-10 h-10 rounded-xl bg-[var(--card-bg)] border border-[var(--border-color)] text-[var(--text-muted)] flex items-center justify-center hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm shrink-0">
                <ChevronLeft size={18} />
            </button>
            <div className="flex-1 w-full">
                <PageBanner 
                    title="Child Categories"
                    subtitle="Manage specific healthcare offerings within subcategories."
                >
                    <button disabled={!selectedSubId} onClick={() => {
                        setEditingItem(null);
                        setName("");
                        setPrice("");
                        setDesc("");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
            <aside className="lg:col-span-1 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-5 shadow-sm text-left space-y-5">
                ...
                <div className="space-y-1.5">
                        ...
                        <select className="w-full h-10 px-3 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg text-xs font-bold outline-none cursor-pointer" value={activeCatId} onChange={e => {
            setActiveCatId(e.target.value);
            setSelectedSubId("");
          }}>
                            <option value="">Choose...</option>
                            {categories?.map(cat => <option key={cat._id} value={cat._id}>{cat.name.replace(/SELECT|ASSIGN/g, "")}</option>)}
                        </select>
                    </div>
                    ...
                    <div className="space-y-2 pt-2 border-t border-[var(--border-color)]">
                        ...
                        <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1">
                            ...
                            {subServices?.map(sub => <button key={sub._id} onClick={() => setSelectedSubId(sub._id)} className={`w-full text-left px-3.5 py-2.5 rounded-lg text-xs font-bold transition-all border ${selectedSubId === sub._id ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 border-emerald-100 dark:border-emerald-500/20" : "text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-main)] border-transparent"}`}>
                ...
                <main className="lg:col-span-3 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-sm">
                    {selectedSubId && <div className="p-4 border-b border-[var(--border-color)] bg-[var(--bg-main)] flex items-center justify-between gap-3">
                            <div style={{
            position: "relative",
            width: "320px",
            flexShrink: 0
              ...
              boxSizing: "border-box"
                    ...
                    {!selectedSubId ? <div className="py-20 text-center flex flex-col items-center justify-center space-y-3">
                            ...
                            <table className="w-full text-left min-w-[750px]">
                                ...
                                <tbody className="divide-y divide-[var(--border-color)]">
                                    {filtered && filtered.length > 0 ? filtered.map((child, index) => <tr key={child._id} className="hover:bg-[var(--bg-main)]/50 transition-colors group">
                                                {/* Index */}
                                                ...
                                                <td className="py-4 px-4">
                                                    <div className="w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600 overflow-hidden border border-blue-100/50 dark:border-blue-500/10">
                                                        {child.imageUrl ? <img src={resolveAssetUrl(child.imageUrl)} alt={child.name} className="w-full h-full object-cover" /> : (() => {
                      ...
                      return <ChildIcon size={18} />;
                                                ...
                                                <td className="py-4 px-4">
                                                    <div>
                                                        ...
                                                        {child.description && <p className="text-[10px] text-[var(--text-muted)] max-w-xs truncate mt-0.5">{child.description}</p>}
                                                    </div>
                                                </td>
                                                {/* Price */}
                                                ...
                                                <td className="py-4 px-4 text-center">
                                                    <button onClick={() => toggleFeaturedMutation.mutate(child._id)} className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-bold tracking-wider uppercase transition-all ${child.isFeatured ? "bg-amber-50 text-amber-700 dark:bg-amber-950/20 dark:text-amber-400" : "bg-slate-100 text-slate-500 dark:bg-white/5 dark:text-slate-400"}`}>
                      ...
                      setIsModalOpen(true);
                                                        ...
                                                        <button onClick={() => setDeleteId(child._id)} className="w-8 h-8 rounded-lg border border-red-100 dark:border-red-950/20 bg-red-50/50 dark:bg-red-950/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all">
                                                            <Trash2 size={13} />
                                                        </button>
                                                    </div>
                                                </td>
