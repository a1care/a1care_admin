...
import { Flame, Globe, Smartphone, Save, CheckCircle2, AlertCircle, ChevronRight, Eye, EyeOff, RefreshCw, Shield, Cpu, CreditCard, Mail, Server, Loader2 } from "lucide-react";
...
function MaskedInput({
  ...
  return <div className="relative flex items-center">
            <input id={id} type={show ? "text" : "password"} className="w-full h-10 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-4 pr-12 text-[var(--text-main)] font-mono text-xs font-semibold placeholder:text-[var(--text-muted)]/70 outline-none focus:border-orange-500 transition-all" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} />
            <button type="button" className="absolute right-4 text-[var(--text-muted)] hover:text-orange-500 transition-colors" onClick={() => setShow(s => !s)}>
...
function FieldRow({
  ...
  return <div className="space-y-1.5 text-left">
            <label htmlFor={id} className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider ml-0.5">
                {label}
            </label>
            {masked ? <MaskedInput id={id} value={value?.toString() ?? ""} onChange={onChange} placeholder={`Enter ${label}`} /> : <input id={id} type={type} className="w-full h-10 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-4 text-[var(--text-main)] font-mono text-xs font-semibold placeholder:text-[var(--text-muted)]/70 outline-none focus:border-orange-500 transition-all" value={value ?? ""} onChange={e => onChange(e.target.value)} placeholder={`Enter ${label}`} />}
...
export function SystemSettingsPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: next => {
      ...
      setStatus({
        ok: true,
        msg: `Synced at ${new Date(next.updatedAt).toLocaleTimeString()}`
      });
    ...
    onError: () => {
      ...
      setStatus({
        ok: false,
        msg: "Save failed — check backend logs."
      });
    }
  });
  const setWebsite = (key: keyof SystemConfig["website"], val: string) => setForm(p => ({
    ...p,
    website: {
      ...p.website,
      [key]: val
  ...
  const setClient = (idx: number, key: keyof MobileFirebaseClient, val: string) => setForm(p => {
    const clients = [...p.clients];
    clients[idx] = {
      ...clients[idx],
      [key]: val
    } as MobileFirebaseClient;
    return {
      ...p,
      clients
    };
  });
  const setEasebuzz = (key: keyof SystemConfig["easebuzz"], val: string) => setForm(p => ({
    ...p,
    easebuzz: {
      ...p.easebuzz,
      [key]: val
  ...
  const setEmail = (key: keyof SystemConfig["email"], val: string | number) => setForm(p => ({
    ...p,
    email: {
      ...p.email,
      [key]: val
  ...
  const setTwilio = (key: keyof SystemConfig["twilio"], val: string) => setForm(p => ({
    ...p,
    twilio: {
      ...p.twilio,
      [key]: val
  ...
  const setAWS = (key: keyof SystemConfig["aws"], val: string) => setForm(p => ({
    ...p,
    aws: {
      ...p.aws,
      [key]: val
  ...
  const setRedis = (key: keyof SystemConfig["redis"], val: string | number) => setForm(p => ({
    ...p,
    redis: {
      ...p.redis,
      [key]: val
  ...
  return (
    <div className="space-y-6 animate-in">
        ...
        <PageBanner 
            ...
            subtitle="System Configurations • API & Key Registry"
        >
            <div className="flex items-center gap-2 shrink-0">
                <button type="button" onClick={async () => {
  const isConfirmed = await confirm({
    title: "Save Settings",
    message: "Save these settings to production? This will affect all running services immediately.",
    confirmText: "Save",
    type: "warning"
  });
  if (isConfirmed) {
...
}} disabled={mutation.isPending} className="h-9 px-4 bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-semibold rounded-xl flex items-center gap-2 shadow-sm disabled:opacity-60 transition-all backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="md:col-span-1 space-y-2">
                    <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-3 shadow-sm space-y-1.5 flex flex-col">
                        ...
                        {tabs.map(t => <button key={t.key} type="button" className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-xs font-bold transition-all text-left w-full ${activeTab === t.key ? "bg-orange-50 dark:bg-orange-500/10 text-orange-600 border border-orange-100 dark:border-orange-500/20" : "text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-main)] border border-transparent"}`} onClick={() => setActiveTab(t.key)}>
                ...
                <div className="md:col-span-3">
                    {isLoading ? <div className="bg-[var(--card-bg)] rounded-xl border border-[var(--border-color)] p-6">
                            <FormSkeleton />
                        </div> : <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-6 md:p-8 shadow-sm">
                            {/* ── Website / JS SDK ── */}
                            {activeTab === "website" && <section className="space-y-6">
                                    ...
                                    <div className="grid sm:grid-cols-2 gap-4">
                                        ...
                                        <FieldRow id="ws-measurementId" label="Measurement ID" value={form.website.measurementId} onChange={v => setWebsite("measurementId", v)} note="Optional — required for Analytics" />
                                    </div>
                                </section>}

                            {/* ── Project Info ── */}
                            {activeTab === "project" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                                    ...
                                    <div className="grid sm:grid-cols-2 gap-4">
                                        <FieldRow id="proj-number" label="Project Number (Sender ID)" value={form.projectNumber} onChange={v => setForm(p => ({
                ...p,
                projectNumber: v
              }))} />
                                        <FieldRow id="proj-id" label="Project ID" value={form.projectId} onChange={v => setForm(p => ({
                ...p,
                projectId: v
              }))} />
                                        <FieldRow id="proj-storage" label="Cloud Storage Bucket" value={form.storageBucket} onChange={v => setForm(p => ({
                ...p,
                storageBucket: v
              }))} />
                                    </div>
                            ...
                            {/* ── Mobile Clients ── */}
                            {activeTab === "clients" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                ...
                return <div key={idx} className="bg-[var(--bg-main)] rounded-xl p-5 border border-[var(--border-color)] space-y-4 text-left">
                            ...
                            {activeTab === "firebase" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                                    ...
                                    <div className="grid gap-4 text-left">
                                        <FieldRow id="fb-email" label="Client Email" value={form.firebase?.clientEmail || ""} onChange={v => setForm(p => ({
                ...p,
                firebase: {
                  ...p.firebase!,
                  clientEmail: v
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <textarea className="w-full h-40 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg p-4 text-[var(--text-main)] font-mono text-[11px] font-semibold outline-none focus:border-orange-500 transition-all" value={form.firebase?.privateKey || ""} onChange={e => setForm(p => ({
                  ...p,
                  firebase: {
                    ...p.firebase!,
                    privateKey: e.target.value
                            ...
                            {/* ── Google Maps ── */}
                            {activeTab === "maps" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                                    ...
                                    <div className="grid gap-4">
                                        <FieldRow id="maps-apiKey" label="Google Cloud API Key" value={form.googleMapsApiKey} onChange={v => setForm(p => ({
                ...p,
                googleMapsApiKey: v
              }))} masked note="Ensure 'Restrictions' in Google Console allow Bundle IDs and Web Domains." />
                                    </div>
                                </section>}

                            {/* ── Easebuzz ── */}
                            {activeTab === "easebuzz" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                                        <div className="w-10 h-10 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-lg flex items-center justify-center">
                                            <CreditCard size={20} />
                                        </div>
                                    ...
                                    <div className="grid sm:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-1.5 sm:col-span-2 text-left">
                                            ...
                                            <div className="flex gap-2">
                                                {["test", "prod"].map(env => <button key={env} type="button" onClick={() => setForm(p => ({
                    ...p,
                    easebuzz: {
                      ...p.easebuzz,
                      env: env as any
                            ...
                            {/* ── Email ── */}
                            {activeTab === "email" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                            ...
                            {/* ── Twilio ── */}
                            {activeTab === "twilio" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                                        <div className="w-10 h-10 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 rounded-lg flex items-center justify-center">
                                            <Mail size={20} />
                                        </div>
                            ...
                            {/* ── AWS ── */}
                            {activeTab === "aws" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                                        <div className="w-10 h-10 bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400 rounded-lg flex items-center justify-center">
                                            <Server size={20} />
                                        </div>
                            ...
                            {/* ── Redis ── */}
                            {activeTab === "redis" && <section className="space-y-6">
                                    <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4 text-left">
                                        <div className="w-10 h-10 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 rounded-lg flex items-center justify-center">
                                            <RefreshCw size={20} />
                                        </div>
