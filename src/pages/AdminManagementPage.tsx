...
export function AdminManagementPage() {
  ...
  return <div className="flex-col gap-8">
            <header className="flex justify-between items-center">
                <div>
                    <div className="w-full mb-6"><PageBanner title="Admin Management" subtitle="Manage administrator accounts and roles" /></div>
      ...
      gap: "24px"
                ...
                {/* Create Admin Form */}
                <div className="card p-8 border-none bg-[var(--card-bg)]" style={{
        borderRadius: "24px"
      }}>
                    ...
                    <form onSubmit={handleSubmit} className="flex-col gap-4">
                        <div className="flex-col gap-1.5">
                            ...
                            <div className="relative">
                                <User size={16} className="absolute text-[var(--text-muted)]" style={{
                left: 14,
                top: "50%",
                transform: "translateY(-50%)"
              }} />
                                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="John Smith" autoComplete="name" className={`w-full bg-[var(--bg-main)] border rounded-xl px-4 py-3 text-sm ${errors.name ? "border-red-400" : "border-[var(--border-color)]"}`} style={{
                paddingLeft: "40px"
              }} />
                            </div>
                        ...
                        <div className="flex-col gap-1.5">
                            ...
                            <div className="relative">
                                <Mail size={16} className="absolute text-[var(--text-muted)]" style={{
                left: 14,
                top: "50%",
                transform: "translateY(-50%)"
              }} />
                                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@a1care.com" autoComplete="email" className={`w-full bg-[var(--bg-main)] border rounded-xl px-4 py-3 text-sm ${errors.email ? "border-red-400" : "border-[var(--border-color)]"}`} style={{
                paddingLeft: "40px"
              }} />
                            </div>
                        ...
                        <div className="flex-col gap-1.5">
                            ...
                            <div className="relative">
                                <Lock size={16} className="absolute text-[var(--text-muted)]" style={{
                left: 14,
                top: "50%",
                transform: "translateY(-50%)"
              }} />
                                <input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 8 characters" autoComplete="new-password" className={`w-full bg-[var(--bg-main)] border rounded-xl px-4 py-3 text-sm ${errors.password ? "border-red-400" : "border-[var(--border-color)]"}`} style={{
                paddingLeft: "40px",
                paddingRight: "44px"
              }} />
                                <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute text-[var(--text-muted)]" style={{
                right: 14,
                top: "50%",
                transform: "translateY(-50%)"
              }}>
                        ...
                        <div className="flex-col gap-1.5">
                            ...
                            <select value={role} onChange={e => setRole(e.target.value as AdminRole)} className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium text-[var(--text-main)]">
                                <option value="admin">Admin</option>
                                <option value="super_admin">Super Admin</option>
                            </select>
                        </div>

                        <button type="submit" disabled={createMutation.isPending} className="button primary w-full h-12 rounded-xl text-sm font-black uppercase tracking-widest mt-2 disabled:opacity-60">
                ...
                {/* Admin List */}
                <div className="card p-0 overflow-hidden border-none bg-[var(--card-bg)]" style={{
        borderRadius: "24px"
      }}>
                    ...
                    {isLoading ? <div className="py-20 text-center">
                        ...
                        </div> : isError ? <div className="py-20 text-center">
                            <p className="text-sm font-bold text-red-500">Failed to load admins. Please refresh.</p>
                        </div> : admins.length === 0 ? <div className="py-20 text-center flex-col items-center gap-3">
                    ...
                    fontSize: "0.9rem"
                                            ...
                                            <td>
                                                <span className="text-xs muted font-medium">
                                                    {(admin as any).createdAt ? new Date((admin as any).createdAt).toLocaleDateString("en-IN", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric"
                    }) : "—"}
                                            ...
                                            <td>
                                                <div className="flex justify-center">
                                                    <button onClick={() => handleDelete(admin)} disabled={deleteMutation.isPending} className="icon-button text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 disabled:opacity-40" title="Remove admin">
                                                        <Trash2 size={15} />
                                                    </button>
                                                </div>
                                            </td>
