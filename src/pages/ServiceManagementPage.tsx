...
import { Plus, Trash2, ChevronRight, LayoutGrid, Layers, Tag, X, Image as ImageIcon, Loader2, MoreVertical, Filter, Search, Globe, Settings2, Star } from "lucide-react";
...
export function ServiceManagementPage() {
  ...
  const [deleteConfig, setDeleteConfig] = useState<{
    id: string;
    type: 'category' | 'subservice' | 'child';
  } | null>(null);
  ...
  const confirmGenericDelete = () => {
    ...
    if (type === 'category') deleteCatMutation.mutate(id);else if (type === 'subservice') deleteSubMutation.mutate(id);else if (type === 'child') deleteChildMutation.mutate(id);
  ...
  // ── Render Helpers ──
  const renderCategories = () => <div className="flex-col gap-4">
            ...
            <div className="grid-4">
                {Array.isArray(categories) && categories.map(cat => <article key={cat._id} className={`card flex-col gap-3 group cursor-pointer ${selectedCatId === cat._id ? "card-primary" : ""}`} onClick={() => {
        ...
        setActiveTab("sub-services");
            ...
            marginBottom: '4px'
                            ...
                            <p className="text-xs muted uppercase tracking-wider font-bold" style={{
            opacity: 0.7
          }}>
                                {cleanType(cat.type)}
                            </p>
          ...
          borderTop: '1px dashed rgba(0,0,0,0.05)'
                            ...
                            <span className="text-xs font-bold items-center gap-1" style={{
            display: 'flex'
          }}>
  ...
  const renderSubServices = () => <div className="flex-col gap-4">
            <div className="section-header">
                <div className="flex items-center gap-3">
                ...
                <button onClick={() => setIsSubModalOpen(true)} className="button primary" disabled={!selectedCatId}>
                    <Plus size={18} /> Add Sub-Service
                </button>
            </div>

            {!selectedCatId ? <div className="card-ghost">
                    <LayoutGrid size={32} style={{
        marginBottom: '12px'
      }} />
                    <p>Select a primary category to view sub-services</p>
                </div> : <div className="grid-3">
                    {Array.isArray(subServices) && subServices.map(sub => <article key={sub._id} className={`card flex-col gap-3 group cursor-pointer ${selectedSubId === sub._id ? "card-primary" : ""}`} onClick={() => {
        setSelectedSubId(sub._id);
        setActiveTab("child-services");
      }}>
                            <div className="flex justify-between items-start">
                                <div className="icon-box">
                                    <Layers size={22} />
            ...
            padding: '6px'
                            ...
                            <div>
                                <h3 className="font-bold" style={{
            fontSize: '1rem'
          }}>{cleanName(sub.name)}</h3>
                                <p className="text-xs muted uppercase font-bold tracking-wider" style={{
            opacity: 0.7
          }}>Specialized Tier</p>
                            </div>
                            <div className="flex items-center justify-between" style={{
          ...
          borderTop: '1px dashed rgba(0,0,0,0.05)'
                    ...
                    {subServices?.length === 0 && !loadingSubs && <div className="card-ghost">No sub-services found for this node.</div>}
                </div>}
        </div>;
  const renderChildServices = () => <div className="flex-col gap-4">
            <div className="section-header">
                <div className="flex items-center gap-3">
                ...
                <button onClick={() => setIsChildModalOpen(true)} className="button primary" disabled={!selectedSubId}>
                    <Plus size={18} /> New Service Item
                </button>
            </div>

            {!selectedSubId ? <div className="card-ghost">
                    <Layers size={32} style={{
        marginBottom: '12px'
      }} />
                    <p>Select a sub-service to view items</p>
                </div> : <div className="flex-col gap-2">
                    {Array.isArray(childServices) && childServices.map(child => <article key={child._id} className="card flex justify-between items-center group" style={{
        padding: '16px 24px'
      }}>
            ...
            height: '40px'
                ...
                margin: 0
              }}>{cleanName(child.name)}</h4>
                                        {child.isFeatured && <span style={{
                fontSize: '0.65rem',
                background: '#fef9c3',
                color: '#854d0e',
                padding: '2px 8px',
              ...
              marginTop: '4px'
                                        ...
                                        {child.rating > 0 && <span className="text-xs" style={{
                color: '#f59e0b'
              }}>★ {child.rating.toFixed(1)}</span>}
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                <button title={child.isFeatured ? "Remove from Popular" : "Mark as Popular in App"} onClick={() => toggleFeaturedMutation.mutate(child._id)} disabled={toggleFeaturedMutation.isPending} style={{
            padding: '8px',
            borderRadius: '8px',
            border: 'none',
            cursor: 'pointer',
            ...
            transition: 'all 0.2s'
                                ...
                                <button onClick={() => setDeleteConfig({
            id: child._id,
            type: 'child'
          })} className="logout-btn" style={{
            padding: '8px',
            color: '#ef4444'
                    ...
                    {childServices?.length === 0 && !loadingChildren && <div className="card-ghost">No bookable items configured.</div>}
                </div>}
        </div>;
  return <div className="flex-col gap-4">
      ...
      marginBottom: '24px'
                ...
                <div>
                    <div className="w-full mb-6"><PageBanner title="Service Hierarchy" subtitle="Global platform service catalog and pricing" /></div>
        ...
        border: 'none'
                    ...
                    <button onClick={() => setActiveTab("categories")} className={`button ${activeTab === "categories" ? "primary" : "secondary"}`} style={{
          height: '36px',
          fontSize: '0.75rem',
          padding: '0 16px',
          borderRadius: '10px'
                    ...
                    <button onClick={() => setActiveTab("sub-services")} className={`button ${activeTab === "sub-services" ? "primary" : "secondary"}`} style={{
          height: '36px',
          fontSize: '0.75rem',
          padding: '0 16px',
          borderRadius: '10px'
                    ...
                    <button onClick={() => setActiveTab("child-services")} className={`button ${activeTab === "child-services" ? "primary" : "secondary"}`} style={{
          height: '36px',
          fontSize: '0.75rem',
          padding: '0 16px',
          borderRadius: '10px'
      ...
      minHeight: '500px'
            ...
            {isCatModalOpen && <div className="modal-overlay">
          ...
          alignItems: 'center'
                        ...
                        <form style={{
          padding: '24px'
        }} className="flex-col gap-4" onSubmit={e => {
          e.preventDefault();
          ...
          createCatMutation.mutate(fd);
            ...
            {/* Sub-Service Modal */}
            {isSubModalOpen && <div className="modal-overlay">
                    <div className="modal-content">
                        <div style={{
          padding: '24px',
          borderBottom: '1px solid #f1f5f9',
          ...
          createSubMutation.mutate(fd);
                            ...
                            <div className="input-group">
                                ...
                                <textarea value={subDesc} onChange={e => setSubDesc(e.target.value)} placeholder="Brief details about this tier..." style={{
              width: '100%',
              padding: '12px',
              borderRadius: '12px',
              background: '#f8fafc',
            ...
            {/* Child-Service Modal */}
            {isChildModalOpen && <div className="modal-overlay">
        ...
        maxWidth: '600px'
          ...
          createChildMutation.mutate(fd);
                            ...
                            <div className="input-group">
                                <label>Description</label>
                                <textarea value={childDesc} onChange={e => setChildDesc(e.target.value)} placeholder="Service details..." style={{
              width: '100%',
              padding: '12px',
              borderRadius: '12px',
              background: '#f8fafc',
              ...
              fontFamily: 'inherit'
                            ...
                            <button disabled={createChildMutation.isPending} className="button primary full-width" style={{
            marginTop: '12px'
          }}>
            ...
            {deleteConfig && <div className="modal-overlay">
                    <div className="modal-content" style={{
        maxWidth: '420px',
        textAlign: 'center',
        padding: '32px 24px'
          ...
          marginBottom: '8px'
        }}>Archive this {deleteConfig.type}?</h3>
          ...
          gap: '12px'
                            ...
                            <button className="button secondary full-width" onClick={() => setDeleteConfig(null)} style={{
            height: '48px'
          }}>
                                Cancel
                            </button>
                            <button className="button primary full-width" onClick={confirmGenericDelete} style={{
            background: '#ef4444',
            color: 'white',
            height: '48px'
          }}>
