...
import { ShieldAlert, Database, FileText, ClipboardList, Search, Calendar, ArrowRight, User, Phone, Clock, Loader2, Lock } from "lucide-react";
...
export function HealthVaultAuditPage() {
  ...
  return <div className="space-y-6 animate-in">
      {/* ── Page Header ── */}
      <PageBanner 
          title="Health Vault Audit" 
          subtitle="Secure Storage Monitoring"
      >
          <div className="flex items-center gap-2 relative z-10 shrink-0">
            <div className="flex items-center gap-1.5 h-9 px-3 bg-white/10 border border-white/20 text-white rounded-lg text-[10px] font-bold whitespace-nowrap tracking-wider uppercase backdrop-blur-sm">
              <Lock size={12} />
      ...
      }].map((kpi, i) => <div key={i} className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-4 flex flex-col gap-3 text-left">
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="px-5 py-3.5 border-b border-[var(--border-color)] bg-[var(--bg-main)] flex flex-row items-center justify-between gap-3">
            ...
            flexShrink: 0
              ...
              <input type="text" placeholder="Search by TxnID, name, phone..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{
              width: "100%",
              height: 42,
              borderRadius: 12,
              paddingLeft: 38,
          ...
          {isLoading ? <div className="py-20 text-center flex flex-col items-center gap-3">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-[var(--border-color)]">
                  {filteredRecords.length > 0 ? filteredRecords.map((record: any) => <tr key={record._id} className="hover:bg-[var(--bg-main)] transition-colors">
                        ...
                        <td className="py-3.5 px-5">
                          <div className="flex flex-wrap gap-1.5 justify-start">
                            {record.prescriptions?.length > 0 && <span className="px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-200 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20 text-[10px] font-semibold rounded">
                                {record.prescriptions.length} Prescriptions
                              </span>}
                            {record.labReports?.length > 0 && <span className="px-2 py-0.5 bg-purple-50 text-purple-700 border border-purple-200 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/20 text-[10px] font-semibold rounded">
                                {record.labReports.length} Lab Reports
                              </span>}
                            {!(record.prescriptions?.length || record.labReports?.length) && <span className="text-[10px] font-bold text-[var(--text-muted)]">No files linked</span>}
                          </div>
                        </td>
